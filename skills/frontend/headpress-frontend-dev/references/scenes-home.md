...
GET /route                        → 首頁 front page（完整 page payload）
...
- `head()` should use site defaults from `/site` or homepage `seo` from `/route`.
...
- Do not use `/wp/v2/` when `/route` or `/collection` already covers the use case.
