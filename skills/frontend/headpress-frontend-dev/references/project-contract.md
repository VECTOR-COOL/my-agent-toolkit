...
| REST API base（Composition） | `https://{CMS_HOST}/wp-json/headless/v1` | `https://example.com/wp-json/headless/v1` |
...
| Backend model | WordPress Headless CMS（HeadPress Theme）| RESTful JSON via `/headless/v1/` |
...
- **`openapi.json` owns API surface**：所有前端消費的 endpoint 與欄位形狀以此為準。
...
- 不要為了滿足 UI component 而改 CMS 欄位名稱；透過 mapper/service 處理，或提交後端 schema change 請求。
- 不要宣稱 domain 已 bind、已部署或已發布，除非平台/部署工具已實際確認。
- 前端操作任何 `/headless/v1/` endpoint 前，先確認該 endpoint 已在 `openapi.json` 中定義。
