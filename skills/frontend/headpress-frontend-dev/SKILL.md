...
version: "1.2.2"
minimum_headpress_version: "0.6.0"
...
[![HeadPress](https://img.shields.io/badge/HeadPress-%3E%3D0.6.0-informational)]()
...
| 項目 | 值 |
| --- | --- |
| 本 skill 最低 HeadPress Theme 版號 | **≥ 0.6.0**（YAML：`minimum_headpress_version`） |
| 整合前驗證 | `GET /health` 或 `GET /manifest` 回傳的 `version` |
| 端點是否存在 | 仍以 runtime `GET /openapi.json` 為準 |

低於 **0.6.0** 時，不得假設本 skill 所列的 `/route`、`/content/*`、`/content-object/{id}`、`POST /query`、`GET /languages` 等端點皆已實作；應升級 HeadPress 或改以 openapi 實際存在的 legacy 端點（如 `/page/{path}`）整合。
...
後端必要條件：CPT 必須 `public: true`、`show_in_rest: true`，並加入 `headpress/route/post_type_allowlist`。若只需要一個公開 publish object 且已知 post type，可在 service layer 使用 `GET /content/{post_type}/{identifier}`；若只知道 WordPress ID、不知道 post type，使用 `GET /content-object/{id}`。這兩個單筆 endpoint 永遠回單一 object 或 404，不回陣列；若需要 SEO、breadcrumb、redirect 或 404 page context，仍使用 `/route/{path}`。需要陣列型條件時才用 `POST /query`，且只能傳 openapi.json `QueryRequest` 支援的 safe WP_Query-shaped subset。語言資訊使用 `GET /languages`；WordPress core 只代表 locale / installed language packs，多語內容需後端 plugin 或子主題定義。
