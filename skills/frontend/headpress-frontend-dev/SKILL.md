...
version: "1.0.1"
description: HeadPress WordPress Headless Theme 前端 UI 開發協作 Skill。用於規劃或修改 HeadPress 驅動的前端 routes、layouts、components、SSR/pre-render、SEO metadata；消費 Composition API（/headless/v1/，禁止直接呼叫 /wp/v2/）；對齊 openapi.json schema、normalized view model、mock-to-api 遷移、service-layer 邊界、環境變數與錯誤處理；撰寫 Lovable、v0、Replit、Cursor 等 AI builder prompt；debug hydration、CORS、auth、404、空資料與媒體 fallback。搭配 wordpress-rest-api-development 或 wordpress-plugin-theme-development 處理 schema 與主題後端。
---
...
**前端唯一資料來源**：`https://example.com/wp-json/headless/v1/`（Composition API）。
前端 **不得** 直接呼叫 `/wp/v2/` 任何 endpoint。
...
前端只消費以下 endpoints（完整 schema 在 `openapi.json`）：
...
前端只需要 **一個 API client** 指向 `/headless/v1/`。

---

### Step 3 — 對齊回應欄位

Composition API 的 `entity`、`items`、media、taxonomy 欄位**保留 WordPress REST conventions**；站台擴充欄位預設放在 `headless` key。
...
- 將 mock data 改成 `/headless/v1/` API，或建立 HeadPress-shaped mock 穩定 UI。
...
- 需要確認 `/headless/v1/` endpoint 的 response schema。
...
- **唯一 API Client**：前端只用一個指向 `/headless/v1/` 的 client；不直接呼叫 `/wp/v2/`。
...
- 不要讓 component 直接呼叫 `/wp/v2/` 或任何 CMS URL（只能透過 headlessClient）。
...
src/services/         # headlessClient、fetch、cache、錯誤處理
  └── headlessClient.ts   # 唯一 API client，指向 /headless/v1/
...
src/data/mock/        # mock fixtures（資料形狀貼近 /headless/v1/ response）
...
- **資料來源**：`/headless/v1/` Composition API；schema 見 `themes/headpress/docs/prd/openapi.json`。
- **可改範圍**：指定 files / routes / components。
- **不可改範圍**：headlessClient、mapper、env、routing contract、全域樣式、SEO utilities。
...
資料來源為 /headless/v1/collection?type=post，schema 見 openapi.json。
headlessClient、mapper、env 設定不可修改。
...
1. 先查 `openapi.json` 的 `/headless/v1/collection` response schema。
2. 定義 normalized item type（例如 `PostCardModel`）。
3. 建立 mock fixture，覆蓋正常、空列表、缺圖、長標題。
4. 建立 headlessClient fetcher 與 mapper（`collection → PostCardModel[]`）。
...
1. 查 `openapi.json` 的 `/headless/v1/page` response schema，確認 slug 對應欄位。
...
3. 確認 `/headless/v1/` CORS 允許前端 production domain。
...
- component 是否繞過 headlessClient 直接呼叫 CMS URL。
- mock 與 `/headless/v1/` response 是否形狀一致，切換後不破版。
