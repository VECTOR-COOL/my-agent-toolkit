...
version: "1.4.0"
description: HeadPress WordPress Headless Theme 前端 UI 開發協作 Skill。用於規劃或修改 HeadPress 驅動的前端 routes、layouts、components、SSR/pre-render、SEO metadata；引導 Agent 理解後端是 WordPress Headless CMS，可使用 WordPress 的 posts/pages/CPT/taxonomy/media/menu/permalink 邏輯；優先消費 Composition API（public base /headpress/api/v1，REST namespace headpress/api/v1）；僅在 HeadPress 無法提供所需資料時才於 service layer 使用 /wp/v2/；對齊 openapi.json schema、normalized view model、mock-to-api 遷移、service-layer 邊界、環境變數與錯誤處理；撰寫 Lovable、v0、Replit、Cursor 等 AI builder prompt；debug hydration、CORS、auth、404、空資料與媒體 fallback。搭配 wordpress-rest-api-development 或 wordpress-plugin-theme-development 處理 schema 與主題後端。
---
...
**核心定位**：HeadPress 前端不是連到任意 headless CMS，也不是自行設計一套資料庫 API。後端是啟用 HeadPress 父主題的 **WordPress Headless CMS**；前端應使用 WordPress 既有內容邏輯（post、page、CPT、taxonomy、term、media、menu、permalink、publish/private status、SEO plugin fields），但預設透過 HeadPress Composition API 取得已組裝、可渲染的 public read-only payload。
...
從零建立 HeadPress 驅動的前端時，按此順序執行：

1. **Discover runtime contract**：先讀 `GET /manifest`，再讀 `GET /openapi.json`；若在 repo 內工作，對照 `themes/headpress/docs/prd/openapi.json`。
2. **設定 environment**：前端公開變數只放 `VITE_SITE_URL` 與 `VITE_WP_API_URL` 這類 base URL；Application Password、token、cookie secret 只放 server-only secrets。
3. **建立 service layer**：建立 `headpressClient` 指向 `/headpress/api/v1`；若必要，另建 `wpClient` 作為 service-layer only fallback。
4. **建立 App Shell**：用 `GET /site` 取得站台 identity、menus、SEO defaults、social links；用 mapper 轉成 Header/Footer 可用模型。
5. **建立 route loader**：首頁呼叫 `GET /route?include=all`；動態路由呼叫 `GET /route?path={current_path}&include=all`。
6. **建立 template mapping**：依 `route.kind`、`route.template`、`route.view` 選擇 `HomePage`、`PageTemplate`、`SinglePostTemplate`、`ArchiveTemplate`、`NotFoundTemplate` 等。
7. **建立 content services**：列表、搜尋、taxonomy archive 才呼叫 `/collection`、`/content/{post_type}`、`/search`、`/taxonomy/*`；不要用 component 直連 `/wp/v2/posts`。
8. **建立 mapper 與 fallback**：將 WordPress-shaped payload 轉為 normalized view model；處理缺圖、空 excerpt、HTML 內容、空列表、404、redirect、API timeout。
9. **落地 SEO/SSR**：公開頁面在 SSR/SSG/pre-render 階段注入 `seo`、canonical、Open Graph、robots 與 JSON-LD schema。
10. **驗收**：測首頁、一般 page、single post/CPT、archive、search、404、redirect、空資料、缺圖、CORS、production env 與 mobile/desktop UI。
...
> **⚠️ 規格鎖定**：請嚴格遵守 [`themes/headpress/docs/spec/openapi/endpoints-and-rules.md`](themes/headpress/docs/spec/openapi/endpoints-and-rules.md) 之 Rendering Contract。
...
**【進階與補充資料】**
- `GET /menus/locations`：取得選單位置列表。
- `GET /theme`：取得設計系統 (CSS變數、顏色、排版)。
- `GET /templates`：前端映射版型清單。
- `GET /content-types`：取得公開的 CPT 與 Taxonomy 結構。
- `GET /collection`、`GET /search`、`GET /sitemap` 等。

*(註：`GET /route/{path}` 是完全受支援的端點，適合手寫前端與人工測試；但若是 AI 生成 SDK，為避免多層路徑的斜線 encode 問題，推薦使用 `?path=`)*
...
| 任務                       | 查閱路徑                                                    |
| ---------------------------- | ------------------------------------------------------------- |
| API 端點清單與使用規則     | `themes/headpress/docs/spec/openapi/endpoints-and-rules.md` |
...
1. **優先使用 SSR / SSG**：
   - 只要平台支援 (如 Next.js、Nuxt、TanStack Start、Replit)，必須優先使用 Server-Side Rendering (SSR) 或 Static Site Generation (SSG)。
   - CSR (Client-Side Rendering) 僅作為最後 fallback 或互動狀態使用，**嚴禁**作為公開 SEO 頁面的唯一渲染方式。
2. **SEO 與 Meta 標籤注入**：
   - `data.seo` 提供完整的 `title`, `description`, `canonical`, `robots`, `resolved_og_image`。必須在 Server 端注入 `<head>`。
   - `data.schema` 的 JSON-LD 必須在 Server 端渲染為 `<script type="application/ld+json">`。
3. **路由狀態處理 (Redirect & 404)**：
   - 當 `data.route.kind === 'redirect'` 或 HTTP Status 為 301 時，前端必須執行 HTTP Redirect 至 `data.route.redirect_to`。
   - 當 `data.route.kind === 'not-found'`，前端應觸發框架的 404 機制 (如 `notFound()`) 或渲染自訂 `NotFoundTemplate`。
4. **元件映射層級 (Component Mapping)**：
   - 頁面版型優先順序：`data.route.template` → `data.route.view`。
   - 內容解析優先順序：`data.sections` (結構化區塊) → `entity.blocks` → `entity.content.rendered` (純 HTML)。
5. **無狀態與唯讀限制 (Stateless & Read-Only)**：
   - HeadPress API **不處理** 表單送出、寄送信件、或會員登入 (Auth/Sessions)。
   - 遇到這些需求時，請使用 WordPress 原生 REST API (如 `/wp/v2/users/me`) 或外部 Serverless Functions，**絕對不要**假定 HeadPress 可以處理寫入操作。
...
```text
GET /route?path=/news-release-2026&include=route,entity,seo,media
...
後端必要條件：CPT 必須 `public: true`、`show_in_rest: true`，並加入 `headpress/route/post_type_allowlist`。若只需要一個公開 publish object 且已知 post type，可在 service layer 使用 `GET /content/{post_type}/{identifier}`；若只知道 WordPress ID、不知道 post type，使用 `GET /content-object/{id}`。這兩個單筆 endpoint 永遠回單一 object 或 404，不回陣列；若需要 SEO、breadcrumb、redirect 或 404 page context，仍必須使用 `GET /route?path={current_path}&include=all`。需要陣列型條件時才用 `POST /collection`，且只能傳 openapi.json `QueryRequest` 支援的 safe WP_Query-shaped subset。語言資訊使用 `GET /languages`；WordPress core 只代表 locale / installed language packs，多語內容需後端 plugin 或子主題定義。
