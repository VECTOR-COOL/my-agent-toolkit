...
# Frontend + WordPress Headless 專案 Skill

[![version](https://img.shields.io/badge/version-1.0.0-blue)]()
[![Website](https://img.shields.io/badge/Website-vector.cool-blue)](https://vector.cool)

這個 skill 用於維護「React/TypeScript 前端或 AI builder 產生的前端」與「WordPress Headless REST API」整合專案。適用平台包含 Lovable、v0、Replit、Cursor 協作專案與一般前端 repo；核心目標是讓 UI 開發、資料串接、SEO、部署與多人/多工具協作都有明確邊界，避免平台生成、人工修改、WordPress 後台與 API 切換互相破壞。
...
4. 若任務是通用前端架構、component 分解、資料層規劃、prompt 組裝、mock/API 策略或 builder 專案拆解，同時載入 `.agents/skills/frontend-development`。
5. 若任務是舊 Lovable React/Vite SPA 搬到 TanStack Start SSR，才同時載入 `.agents/skills/lovable-legacy-to-ssr-migration`。
6. 若任務依賴 Lovable、v0、Replit 或部署平台的即時行為，重新確認官方文件；平台能力和預設技術棧可能變動。

## 使用時機

在下列情境使用本 skill：

- 規劃或修改 WordPress-backed 前端專案架構。
- 建立、調整或搬移 routes、pages、layouts、components、data services。
- 將 mock data 改成 WordPress REST API，或反向建立 WordPress-shaped mock 以穩定 UI。
- 定義 WordPress post type、ACF 欄位、REST response mapping、TypeScript 型別。
- 處理 SSR、SSG、pre-render、SEO metadata、canonical、Open Graph、custom domain、production build 準備。
- 設計或修補 API error、timeout、CORS、auth、not found、empty data、fallback media、WordPress 維護中等錯誤處理。
- 檢查 AI builder 或 coding agent 產出的程式是否破壞既有架構、樣式、資料層或 SEO。
- 撰寫給 Lovable、v0、Replit、Cursor、WordPress 管理者使用的分工規則或 prompts。
- Debug hydration mismatch、API fallback、空資料、圖片缺失、路由 404、部署環境變數等問題。

若任務重點不是 WordPress Headless，而是一般前端產品或 UI 開發，優先使用 `frontend-development`。若任務重點是 WordPress REST API schema、mock 範本或 service 實作細節，搭配 `wordpress-rest-api-development`。

## 核心原則

- **保留現有 stack**：先辨識專案實際使用的 framework、router、rendering mode、package manager 與部署平台；除非使用者明確要求，不要把 Lovable、v0、Replit 或一般 repo 轉成另一個 stack。
...
- **SSR/SEO 依專案能力落地**：需要 SEO 的內容應在 server render、static generation、pre-render 或平台支援的爬蟲可見流程取得；不要只靠 client effect 補主要內容。
- **型別先行**：所有 CMS response、normalized model、route loader 或 server function 回傳值都要有 TypeScript 型別或 schema。
- **可缺資料設計**：WordPress 後台資料常缺圖、缺 excerpt、欄位為 null、slug 改名、狀態 unpublished；UI 與 mapper 必須有 fallback。
- **錯誤處理前置**：API error、timeout、CORS、auth、not found、rate limit、WordPress 維護中與欄位缺失，都要在 service / mapper / route loader 邊界正規化，再交給 UI 呈現。
- **AI builder 只改可控範圍**：交給 Lovable、v0、Replit 或其他 agent 產出 UI 或局部改版時，要明確指定不可改動 data services、routing contract、env schema、SEO utilities 與 shared types。
...
- 不要把 SSR/SSG/pre-render 能力改成 CSR-only，除非專案本來就沒有 SEO 要求且使用者接受。
...
- 不要為了通過 build 關掉 TypeScript、ESLint、SSR/SSG、route loader 或錯誤處理。
- 不要讓 AI builder 或 agent 直接修改 `.env`、secret、auth token、WordPress application password。
...
- `src/routes/` 或 `app/`：framework routes、loader、page shell。
...
- `src/data/mock/`：mock fixtures，資料形狀要貼近 WordPress raw response 或明確標示 normalized mock。
- `src/config/`：env parsing、site config、API endpoints。
- `docs/`：交接文件、AI-builder prompts、WordPress 欄位規格、部署檢查表。

不要把 fetch、mapping、fallback、UI rendering 全塞在同一個 component；這會讓後續 UI 生成或手動改版很容易破壞資料邏輯。
...
WordPress Headless 專案必須把錯誤處理放在資料邊界，而不是讓 UI component 解析 raw response 或例外物件。
...
- service layer 要把錯誤正規化成 typed result 或可預期的 route error；component 只接收 normalized success/loading/empty/error/notFound/unauthorized/forbidden 狀態。
- mapper 要處理 `null`、缺欄位、空陣列、缺 `_embedded`、缺 featured media、unpublished/private item、slug 找不到或多筆 slug match。
- dynamic route 找不到內容時要回傳專案框架支援的 404/not-found，不要渲染空白 detail page 或錯誤內容。
- API 失敗時可使用 mock 或快取 fallback 只限於明確標示的 degraded state，不可讓使用者誤以為看到的是最新正式內容。
- protected preview/draft/auth flow 要區分 unauthorized、forbidden、expired token、missing env，不可 fallback 到公開內容掩蓋錯誤。
- error UI 要保留 app shell、導覽與可恢復操作；不可整頁 blank，也不可把錯誤細節或 secret 顯示給使用者。
- 測試或 fixture 至少覆蓋正常資料、空資料、API error、找不到 slug、欄位缺失、缺圖與 auth/CORS 類錯誤。
...
- production build 是否能完成目標 rendering mode。
...
- 文章、頁面、分類、產品、案例等可索引內容，應在 framework 支援的 server-side、static、loader、server function 或 pre-render 流程取得。
...
## UI 與 AI Builder 協作規則

交給 Lovable、v0、Replit、Cursor 或其他 agent 的 prompt 應明確限制：
...
- 平台專屬檔案、build config、生成檔、部署設定只能在任務需要且已確認用途時修改。

檢查 AI builder 產出時，優先看：

- 是否新增了重複或衝突的 mock data。
- 是否 component 直接 fetch WordPress。
- 是否刪掉 SSR/SSG/pre-render 資料流程或 metadata。
- 是否導入不必要套件。
- 是否用瀏覽器專用 API 造成 SSR build 或 static generation 失敗。
...
- 辨識目前平台：Lovable、v0、Replit、自架 Vite/Next/TanStack、或其他。
...
- 避免在 SSR/static path 使用 `window`、`document`、`localStorage`、`location`，除非有 client-only guard。
...
- 檢查是否誤改 unrelated files、平台生成檔或使用者未要求的全域樣式。
...
- render：server/static HTML、hydrated client 畫面。
...
2. 建立 WordPress-shaped mock fixture，覆蓋正常、空列表、缺圖、長標題。
3. 建立 WordPress fetcher 與 mapper。
4. 在 route loader、server function、server component 或專案現有資料流程取得資料，component 只負責 render。
...
3. 檢查 build、route 404、redirect、asset path。
4. 確認 WordPress REST API 可由 production domain 存取。
5. 若尚未實作 sitemap/robots/schema，明確列為後續工作，不要混在 UI 任務中硬做。
6. 若平台提供 Publish、Preview、SEO scan 或 domain binding，確認是否需要平台端操作，不要只改 repo code 就聲稱完成上線。

## Prompt 撰寫格式

交給 AI builder 或 coding agent 的 prompt 建議包含：

- 專案階段：UI mock、API migration、SEO hardening、production readiness。
- 平台與 stack：例如 Lovable TanStack Start、v0/Next.js、Replit React app，或 active repo 實際架構。
- 可改範圍：指定 files / routes / components。
- 不可改範圍：資料服務、mapper、env、routing contract、全域樣式等。
- 資料規格：WordPress raw response 或 normalized model 欄位、空資料規則、圖片 fallback。
- 驗收條件：desktop/mobile、loading/empty/error、SSR/SEO 或 static HTML、build/typecheck。
...
- Build 是否會因 browser-only API、dynamic import、env 缺失而失敗。
...
- AI builder 是否產生了無關重構、全域 CSS 污染或套件膨脹。
