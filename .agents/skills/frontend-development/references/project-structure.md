...
以下是現代 React/TypeScript 前端 app 的預設目標架構，適用於 Lovable、v0、Replit 或本地 codebase。若 repo 已有更明確且一致的 convention，優先沿用既有 convention。
...
- `docs/prompts`: AI builder prompts、Knowledge file drafts、prompt history、negative prompts。
- `docs/qa`: visual、SEO、release、migration checklists。
- `scripts/`: 可重複執行的 deterministic automation。React/TypeScript 專案優先用 Node `.mjs`。
